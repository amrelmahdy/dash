...
class Products extends Component {
  ...
  componentDidMount = async () => {
    ...
    this.setState(
      {
        currentCategory: this.props.route.params.category,
      },
      () => {
  ...
  updateSearch = search => {
    ...
    if (search === '') {
      ...
      this.props.getProducts(
        this.state.currentCategory ? this.state.currentCategory.id : -1,
      );
      this.setState({
        updateSearch: false,
      });
  ...
  handleLoadMore = () => {
    ...
    setTimeout(async () => {
      await this.props.getMoreProducts(
        !this.state.currentCategory ? -1 : this.state.currentCategory.id,
        this.state.search,
        this.state.limit,
        ...
        {this.props.isOfflineConnection ? (
          ...
          <FlatList
            ...
            renderItem={({item, index}) => (
              <ProductCell search={false} index={index} product={item} />
            )}
